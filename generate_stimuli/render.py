...
def render_vis(
    ...
    with tf.Graph().as_default() as graph, tf.Session() as sess:
        ...
        loss, loss_per_image, vis_op, t_image, loss_additional_global = (
            T("loss"),
            T("loss_per_image"),
            T("vis_op"),
            T("input"),
            T("loss_additional_global"),
        )
        tf.global_variables_initializer().run()

        initial_image = t_image.eval()

        images = []
        losses_per_image = []
        loss_additional_global_list = []
        try:
            for i in range(max(thresholds) + 1):
                loss_, loss_per_image_, _, loss_additional_global_ = sess.run(
                    [loss, loss_per_image, vis_op, loss_additional_global]
                )
                if i in thresholds:
                    vis = t_image.eval()
                    images.append(vis)
                    losses_per_image.append(loss_per_image_)
                    loss_additional_global_list.append(loss_additional_global_)
                    if verbose:
                        print_objective_func(sess)
        except KeyboardInterrupt:
            log.warning("Interrupted optimization at step {:d}.".format(i + 1))

        return initial_image, images, losses_per_image, loss_additional_global_list
...
def make_vis_T(
    ...
    objective_additional_global = objectives.as_objective(objective_additional_global)
    ...
    if relu_gradient_override:
        with gradient_override_map(
            {"Relu": redirected_relu_grad, "Relu6": redirected_relu6_grad}
        ):
...
def make_t_image(param_f):
    ...
    if not isinstance(t_image, tf.Tensor):
        raise TypeError(
            "param_f should produce a Tensor, but instead created a "
            + str(type(t_image))
        )
    elif t_image.graph != tf.get_default_graph():
        raise TypeError(
            """param_f produced a t_image tensor belonging to a graph
...
def make_optimizer(optimizer, args):
    if optimizer is None:
        ...
        raise (
            "Could not convert optimizer argument to usable optimizer. "
            "Needs to be one of None, function from (graph, sess) to "
            "optimizer, or tf.train.Optimizer instance."
        )


def import_model(model, t_image, t_image_raw=None):
