# modified version: return objectives per image and not per batch.
#
...
def channel(layer, n_channel, batch=None):
    ...
    if batch is None:
        return lambda T: tf.reduce_mean(T(layer)[..., n_channel], axis=(1, 2))
    else:
        return lambda T: tf.reduce_mean(T(layer)[batch, ..., n_channel], axis=(1, 2))
...
def L2(layer="input", constant=0, epsilon=1e-6, batch=None):
    ...
    if batch is None:
        ...
        return lambda T: tf.sqrt(
            epsilon + tf.reduce_sum((T(layer)[batch] - constant) ** 2)
        )
